...
grid.env <- grid_topo %>%
  st_drop_geometry() %>%
  as.data.frame()
rownames(grid.env) <- grid.env$grid_id # on utilise la colonne grid_id comme rownames
...
env_stat <- data.frame('Moyenne' = apply(grid.env2, 2, mean, na.rm = TRUE),
                          'Écart-type' = apply(grid.env2, 2, sd, na.rm = TRUE),
                          'Étendue' = apply(grid.env2, 2, max, na.rm = TRUE)-apply(grid.env2, 2, min, na.rm = TRUE),
                          'Cov' = (apply(grid.env2, 2, sd, na.rm = TRUE)/apply(grid.env2, 2, mean, na.rm = TRUE))*100) %>%
